...
st.set_page_config(
    page_title="Dashboard Comercio Mundial",layout="wide",initial_sidebar_state="expanded")
...
def load_data():
    ...
    df = pd.read_csv("/content/cleaned_world_export_import.csv")
...
with tab2:
    ...
    with graph_col:
        ...
        fig_country = px.bar(df_country, x="Year", y=columna, color=columna, color_continuous_scale="Blues",title=f"Evolución de {columna} de {selected_country}")


        fig_country.update_layout(paper_bgcolor="#0D1B2A",plot_bgcolor="#0D1B2A",font=dict(color="white"),title=dict(font=dict(color="white"))
            )
